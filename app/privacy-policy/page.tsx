...
export default function PrivacyPolicyPage() {
  const [policyData, setPolicyData] = useState<PrivacyPolicyDocument | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchPolicy = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/legal/privacy-policy`);
        const data = await response.json();

        if (data.success) {
          setPolicyData(data.document);
        }
      } catch (error) {
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-4xl">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">
                  {policyData?.title || 'Politique de Confidentialité'}
                </h1>
                {policyData && (
                  <div className="flex items-center gap-4 text-sm text-gray-400 mt-1">
                    <span className="flex items-center gap-1">
                      <Calendar className="w-4 h-4" />
                      Mise à jour: {new Date(policyData.lastUpdated).toLocaleDateString('fr-FR')}
                    </span>
        ...
        <div className="card-emile prose prose-invert max-w-none">
          {policyData ? (
            ...
            <p className="text-gray-400">Impossible de charger la Politique de Confidentialité.</p>
          )}
        </div>
